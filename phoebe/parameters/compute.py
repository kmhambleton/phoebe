...
def phoebe(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [BoolParameter(visible_if='ds_has_enabled_feature:gp_*', qualifier='gp_scale_fluxes', value=kwargs.get('gp_scale_fluxes', True), copy_for={'kind': ['lc'], 'dataset': '*'}, dataset='_default', description='Whether to scale fluxes for GP computation. Will use the StandardScaler() in sklearn to transform fluxes to a mean of 0 and stdev of 1.')]
